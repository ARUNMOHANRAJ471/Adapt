...
export default class EditStage extends React.Component {
  ...
  updateComponent(e, a) {
    if (a.value != null) {
      ...
      $.ajax({
        url: '/gameadmin/viewStageDetails',
        type: 'POST',
        data: {searchQuery: res},
        success: function(dataDB)
        {
          let data = dataDB.records[0]._fields[0].properties;
          context.setState({StageId: dataDB.records[0]._fields[0].identity.low});
          context.setState({Stage: data.name, Video: data.Video});
        }.bind(this),
        error: function(err)
        {
        }.bind(this)
        ...
        <Grid>
          <Grid.Row>
            ...
            <Grid.Column width={14}>
              <p style={{fontSize: '16px', fontFamily: 'arial'}}><b>Modify Existing Stage</b></p>
              <Form>
                <Form.Field>
                  <label>
                    <p style={{fontSize: '14px', fontFamily: 'arial'}}>Stage to be modified</p>
                  </label>
                  <Dropdown onChange={this.updateComponent} placeholder='Select the Stage to be modified' fluid search selection options={this.state.componentArray}/>
                </Form.Field>
                {/* <Form.Field>
                  <label>
                ...
                <Form.Field>
                  ...
                  <input autoComplete='off' type='text' value={this.state.Stage} onChange={this.Stage} ref='modifiedComponentN' placeholder='Stage name' required/>
                </Form.Field>
                ...
                <Form.Field>
                  ...
                  <input autoComplete='off' type='text' value={this.state.Video} onChange={this.Video} ref='errormsg' placeholder='Video path' required/>
                </Form.Field>
