...
export default class DeleteTheme extends React.Component {
  constructor() {
    super();
    this.state = {
      domain: [],
  ...
  componentWillMount() {
    ...
    $.ajax({
      ...
      success: function(data)
      {
        ...
        domainArray.push({key: 'unlinked_themes', value: 'unlinked_themes', text: 'unlinked_themes'});
        this.setState({
          domain: domainArray
        });
      }.bind(this),
  ...
  checkForThemeDeleteAlert(scenarioToDelete) {
    this.refs.asd.success(
      scenarioToDelete + ' Theme is deleted successfully',
      '', {
        timeOut: 3000,
        extendedTimeOut: 3000
      }
    ...
    return (
      <div>
        <Dimmer active={this.state.dsModal} onClickOutside={this.handleNoDeleteScenarioClick.bind(this)} page style={{fontSize: '130%'}}>
          ...
          <p style={{marginRight: '3.2%'}}>Are you sure you want to delete the selected theme?</p>
        ...
        <Grid>
          <Grid.Row>
            ...
            <Grid.Column width={14}>
              <p style={{fontSize: '16px', fontFamily: 'arial'}}><b>Delete Theme</b></p>
              <Form>
                <Form.Field>
                  <label>
                    <p style={{fontSize: '14px', fontFamily: 'arial'}}>category</p>
                  </label>
                  <Dropdown onChange={this.updatesearchQueryScenario} placeholder='Select the category' fluid search selection options={this.state.domain}/>
                </Form.Field>
                <Form.Field>
                  <label>
                    <p style={{fontSize: '14px', fontFamily: 'arial'}}>Theme</p>
                  </label>
                  <Dropdown onChange={this.updateScenario} fluid placeholder='Select Theme to Delete' selection options={this.state.scenario}/>
                </Form.Field>
                {desc}
              </Form>
