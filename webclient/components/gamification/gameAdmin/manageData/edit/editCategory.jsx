...
export default class EditCategory extends React.Component {
  ...
  getAllDomain() {
    console.log('inside getalldomain');
    let context = this;
    let arrOfComponent = [];
    let arrayadd = [];
    $.ajax({
      url: '/gameadmin/getAllCategory',
      type: 'GET',
    ...
    if (a.value != null) {
      ...
      $.ajax({
        ...
        success: function(dataDB)
        {
          ...
          context.setState({name: data.name});
        }.bind(this),
        error: function(err)
        {
        }.bind(this)
        ...
        <Grid>
          <Grid.Row>
            ...
            <Grid.Column width={14}>
              <p style={{fontSize: '16px', fontFamily: 'arial'}}><b>Modify existing category</b></p>
              <Form>
                <Form.Field>
                  <label>
                    <p style={{fontSize: '14px', fontFamily: 'arial'}}>Category</p>
                  </label>
                  <Dropdown onChange={this.updateComponent} placeholder='Select the Category to be changed' fluid search selection options={this.state.domainArray}/>
                </Form.Field>
                <Form.Field>
                  <label>
                    <p style={{fontSize: '14px', fontFamily: 'arial'}}>Category name</p>
                  </label>
