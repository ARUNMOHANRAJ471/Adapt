...
var SortableItem = SortableElement(({
  ...
  padding: '5px'
}}><Card category={category} cardColor={cardColor} name={name} description={description} place='right' remove={remove} position={position}/></div>);
  ...
  return (
    <div>
      {items.map(function(value, index) {
        ...
        return (<SortableItem key={`item-${index}`} index={index} position={value.position} category={value.category} cardColor={value.cardColor} errorMsg={value.errorMsg} description={value.description} name={value.name} remove={remove}/>)
      })
}
    </div>
  );
